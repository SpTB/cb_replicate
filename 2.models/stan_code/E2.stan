...
//  array[N, Tmax] real con_x_judge_centered;
  for (r in 1:N) {
    for (c in 1:Tmax) {
      ...
      if (dec_type[r,c]>=0) {
           if(judge_type[r,c]==dec_type[r,c]) congruency_centered[r,c] = 0.5; else congruency_centered[r,c]=-0.5;
        } else {
          congruency_centered[r,c] = 0;
        }
    }//t-loop
  }
}
...
transformed parameters{
 ...
 // mu_jSUM =  mu_a[12]/(mu_a[12]+mu_b[12]) * 5;
  ...
  //mu_jCBcondom =  mu_a[17]/(mu_a[17]+mu_b[17]) *10 - 5;
  for (i in 1:N) {
    //val alpha
...
generated quantities {
  ...
  //real <lower=-0.5, upper=0.5> mu_conf;
  ...
  //array [N, Tmax] int is_discrete_regen;
