...
data {
  ...
  //xtra learn pars
  real <lower=-1, upper=1> fixVAF;
  real <lower=-1, upper=1> fixPAF;
  ...
  array[N, Tmax] int <lower=-2, upper = Nstim>  stimR; // which stimulus was displayed on the right...
  array[N, Tmax] int <lower=-2, upper = Nstim>  stimL;  //... and left side
...
transformed data{
  ...
  for (r in 1:N) {
    for (c in 1:Tmax) {
      ...
      if (dec_type[r,c]>=0) {
           if(judge_type[r,c]==dec_type[r,c]) congruency[r,c] = 1;
        } else {
          congruency[r,c] = 0;
        }
    }//t-loop
...
parameters{
  ...
  //Hyper pars
  vector <lower=0> [19] mu_a_raw;
  vector <lower=0> [19] mu_b_raw;
  ...
  //xtra learn
  vector  <lower=0, upper=1>[N] alphaV_fcd_raw;
  vector  <lower=0, upper=1>[N] alphaP_fcd_raw;
}

transformed parameters{

  //un-raw pars
  vector <lower=1> [19] mu_a;
  vector <lower=1> [19] mu_b;
  ...
  vector <lower=0, upper=5>  [N] jCBcon; //XX! update: only positive
  vector <lower=0, upper=5>  [N] jCBdom;
  vector <lower=0, upper=.25>  [N] alphaV_fcd;
  vector <lower=0, upper=.25>  [N] alphaP_fcd;
  ...
  //force learn
  real <lower=0, upper=.25> mu_alphaV_fcd;
  real <lower=0, upper=.25> mu_alphaP_fcd;
  ...
  mu_tempP      =  (mu_a[19]/(mu_a[19]+mu_b[19])) *14+1;
  mu_precV      = (mu_a[6]/(mu_a[6]+mu_b[6])) * 40 + 10;
  mu_precP      = (mu_a[18]/(mu_a[18]+mu_b[18])) * 40 + 10;
  ...
  mu_alphaV_fcd = mu_a[16]/(mu_a[16]+mu_b[16]) *.25;
  mu_alphaP_fcd = mu_a[17]/(mu_a[17]+mu_b[17]) *.25;
  ...
  for (i in 1:N) {
    ...
    //xtra
    tempV[i] = tempV_raw[i] * 14 + 1;
    tempP[i] = tempP_raw[i] * 14 + 1;
     precV[i] = precV_raw[i] * 40 + 10;
     precP[i] = precP_raw[i] * 40 + 10;
  //zoib
    ...
    if (fixVAF == -1) alphaV_fcd[i] = alphaV_fcd_raw[i]*.25; else alphaV_fcd[i] = fixVAF;
    if (fixPAF == -1) alphaP_fcd[i] = alphaP_fcd_raw[i]*.25; else alphaP_fcd[i] = fixPAF;
  } //N loop
...
model {
  ...
  mu_b_raw[16:17]~ gamma(1,.1);//alphas fcd
  // ambivalent on BIDIRECTIONAL
  mu_b_raw[5:8] ~ gamma(1,1); //temp, prec, d0, j0
  mu_b_raw[18:19] ~ gamma(1,1);//other temp prec
  ...
  for (i in 1:N) {
    ...
    tempP_raw       [i] ~ beta(mu_a[19], mu_b[19]);
    precV_raw       [i] ~ beta(mu_a[6],mu_b[6]);
    precP_raw       [i] ~ beta(mu_a[18],mu_b[18]);
   // inc_raw       [i] ~ beta(mu_a[15],mu_b[15]);
    ...
    alphaV_fcd_raw [i] ~ beta(mu_a[16],mu_b[16]);
    alphaP_fcd_raw [i] ~ beta(mu_a[17],mu_b[17]);
    ...
    for (t in 1:Ntrial[i]) {
      ...
      if (choiceLR[i,t]>0) { // only if choice was made
        if (dec_type[i,t] == 0) {
          ev[1] = evV[stimL[i,t]];
          ev[2] = evV[stimR[i,t]];
          choiceLR[i, t] ~ categorical_logit(tempV[i] * ev);
        } else if (dec_type[i,t] == 1) {
          ev[1] = evP[stimL[i,t]];
          ev[2] = evP[stimR[i,t]];
          choiceLR[i, t] ~ categorical_logit(tempP[i] * ev);
        }
      }
      ...
      if (dec_type[i,t] == 0) {
        ...
        //forced
      } else if (dec_type[i,t] == 2) {
        if (choiceLR[i,t] == 1) {  //left
          evP[stimL[i,t]] += ((1-evP[stimL[i,t]]) * alphaP_fcd[i]) ;
          evP[stimR[i,t]] -= ((evP[stimR[i,t]]) * alphaP_fcd[i])  ;
          evV[stimL[i,t]] += ((1-evV[stimL[i,t]]) * alphaV_fcd[i]) ;
          evV[stimR[i,t]] -= ((evV[stimR[i,t]]) * alphaV_fcd[i])  ;
        } else if (choiceLR[i,t] == 2) {
          evP[stimL[i,t]] -= ((evP[stimL[i,t]]) * alphaP_fcd[i])  ;
          evP[stimR[i,t]] += ((1-evP[stimR[i,t]]) * alphaP_fcd[i]) ;
          evV[stimL[i,t]] -= ((evV[stimL[i,t]]) * alphaV_fcd[i])  ;
          evV[stimR[i,t]] += ((1-evV[stimR[i,t]]) * alphaV_fcd[i]) ;
        }
      }
      //judge
      if (judge_type[i,t] == 0) {
        diff = evV[stimR[i,t]] - evV[stimL[i,t]];
        summ = evV[stimR[i,t]] + evV[stimL[i,t]] -mean(evV)*2;
      } else if (judge_type[i,t] == 1) {
        diff = evP[stimR[i,t]] - evP[stimL[i,t]];
        summ = evP[stimR[i,t]] + evP[stimL[i,t]] - mean(evP)*2;
      };
    ...
    for (t in 1:Ntrial[i]) {
      ...
      if (choiceLR[i,t]>0) {
        if (dec_type[i,t] == 0) {
          ...
          log_lik[i,t] += categorical_logit_lpmf(choiceLR[i,t] | tempV[i] * ev);
          d_pred[i,t] = categorical_rng(softmax(tempV[i] * ev));
        } else if (dec_type[i,t] == 1) {
          ev[1] = evP[i,stimL[i,t]];
          ev[2] = evP[i,stimR[i,t]];
          log_lik[i,t] += categorical_logit_lpmf(choiceLR[i,t] | tempP[i] * ev);
          d_pred[i,t] = categorical_rng(softmax(tempP[i] * ev));
        }
      ...
      if (dec_type[i,t] == 0) {
        ...
        if (choiceLR[i,t] == 1) {  //left
          evP[i,stimL[i,t]] += (1-evP[i,stimL[i,t]]) * alphaP_fcd[i] ;
          evP[i,stimR[i,t]] -= (evP[i,stimR[i,t]]) * alphaP_fcd[i]  ;
          evV[i,stimL[i,t]] += (1-evV[i,stimL[i,t]]) * alphaV_fcd[i]  ;
          evV[i,stimR[i,t]] -= (evV[i,stimR[i,t]]) * alphaV_fcd[i]   ;
        } else if (choiceLR[i,t] == 2) {
          evP[i,stimL[i,t]] -= (evP[i,stimL[i,t]]) * alphaP_fcd[i]  ;
          evP[i,stimR[i,t]] += (1-evP[i,stimR[i,t]]) * alphaP_fcd[i] ;
          evV[i,stimL[i,t]] -= (evV[i,stimL[i,t]]) * alphaV_fcd[i]  ;
          evV[i,stimR[i,t]] += (1-evV[i,stimR[i,t]]) * alphaV_fcd[i] ;
        }
      }

      //judge
      if (judge_type[i,t] == 0) {
        diff = evV[i,stimR[i,t]] - evV[i,stimL[i,t]];
        summ = (evV[i,stimR[i,t]] + evV[i,stimL[i,t]])-mean(evV[i,])*2; //centering
      } else if (judge_type[i,t] == 1) {
        diff = evP[i,stimR[i,t]] - evP[i,stimL[i,t]];
        summ = (evP[i,stimR[i,t]] + evP[i,stimL[i,t]])-mean(evP[i,])*2;
      }

      if (judge_type[i,t]>-2 && abs(judgement[i,t])<=1) { // only if judgement made on the trial AND it's not at the bound
