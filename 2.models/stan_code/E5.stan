//#include /pre/license.stan
  //changes: INCLUDE REFERENCE DEPENDENCE
  // NO MORE j0 parameter <- it's identical to jPOS!
...
data {
  ...
  real <lower=-1, upper=1> fixVBC;                   //fix beta con? No -> -1, Yes -> input value
  real <lower=-1, upper=1> fixVBI;                   //fix beta inc? No -> -1, Yes -> input value
  ...
  real <lower=-1, upper=1> fixPBC;                   //fix beta con? No -> -1, Yes -> input value
  real <lower=-1, upper=1> fixPBI;                   //fix beta inc? No -> -1, Yes -> input value
  ...
  real <lower=-1> fixJCBcondom;

  //ref bias
  real <lower=-1> fixJPOS;
  real <lower=-1> fixJPOSdom;
  ...
  array[N, Tmax] real  <lower=-2, upper=1>   ref; // reference (-2 -> NA; 0-> left; 1-> right)
...
transformed data{
  array[N, Tmax] real judgement_transformed; //towards reference
  array[N, Tmax] int is_discrete; //whether the value is discrete
  array[N, Tmax] real congruency_centered;
  array[N, Tmax] real con_x_judge_centered;
  for (r in 1:N) {
    for (c in 1:Tmax) {
      ...
      congruency_centered[r,c] = judge_type[r,c]==dec_type[r,c] - .5;
      con_x_judge_centered[r,c] = (judge_type[r,c]==dec_type[r,c]) * judge_type[r,c]-.25;
    }//t-loop
  }
}
...
parameters{
  ...
  vector <lower=0, upper=1> [N] jPOS_raw; //positive reference
  vector <lower=0, upper=1> [N] jPOSdom_raw;
...
transformed parameters{
  ...
  vector <lower=-5, upper=5>  [N] jDxS;
  vector <lower=-5, upper=5>  [N] jCxS;

  vector <lower=0, upper=5>  [N] jCBcon;
  vector <lower=0, upper=5>  [N] jCBdom;

  vector <lower= 0, upper=5>  [N] jPOS;
  vector <lower=0, upper=5>  [N] jPOSdom;
  ...
  real <lower=0, upper=5> mu_jPOS;
  real <lower=0, upper=5> mu_jPOSdom;
 real <lower=1, upper=15> mu_tempP;
  ...
  mu_jPOS =  mu_a[16]/(mu_a[16]+mu_b[16]) *5;
  mu_jPOSdom =  mu_a[17]/(mu_a[17]+mu_b[17]) *5;
  ...
  for (i in 1:N) {
   ...
   // if (fixINC  == -1) inc[i] = inc_raw[i] * .5; else inc[i] = fixINC;
  //zoib
    ...
    if (fixJPOS == -1)   jPOS[i] = jPOS_raw[i] * 5;  else jPOS[i] = fixJPOS;
    if (fixJPOSdom == -1)   jPOSdom[i] = jPOSdom_raw[i] *5;  else jPOSdom[i] = fixJPOSdom;
...
model {
  ...
  mu_b_raw[11:17] ~ gamma(1,.1); //jCB, jDxS, jCxS jCBdom jCBcon, jPOS, jPOSdom
  // ambivalent on BIDIRECTIONAL
  mu_b_raw[5:8] ~ gamma(1,1); //temp, prec, d0, j0
  mu_b_raw[18:19] ~ gamma(1,1);
  ...
  for (i in 1:N) {
    ...
    jPOS_raw[i] ~ beta(mu_a[16],mu_b[16]);
    jPOSdom_raw[i] ~ beta(mu_a[17],mu_b[17]);
    ...
    for (t in 1:Ntrial[i]) {
      ...
      //judge (calculated towards right item)
      if (judge_type[i,t] == 0) {
      ...
      if (judge_type[i,t]>-2  && abs(judgement[i,t])<=1) { // only on judgement trials
        ...
        if (is_discrete[i,t] == 0) {
          real post = jPOS[i] + jPOSdom[i]*(judge_type[i,t]==0); //trial-lvl ref bias
          real cbt; //trial-lvl c-bias
          if (dec_type[i,t]<0) {
            cbt = 0;
          } else {
            cbt = jCB[i] + jCBdom[i] * (judge_type[i,t]==0)+ jCBcon[i] *(dec_type[i,t]==judge_type[i,t]);// + jCBcondom[i] * con_x_judge[i,t]; //trial-specific CB
                    ...
                    // ref bias //domain effect on ref bias   //difficulty //etc...
          real j_mean = jDIF[i]*diff + jDxS[i]*(diff*summ)+ cbt*(choiceLR[i,t]-1.5) + jCxS[i]*((choiceLR[i,t]-1.5)*summ);

        //FLIP BY REFERENCE!! (but don't flip post, which is always up)
          if (ref[i,t]==0) j_mean = (-j_mean + post); else j_mean = j_mean+post;
          if (judge_type[i,t]==0) {
...
generated quantities {
  ...
  for (i in 1:N) {
    for (t in 1:Ntrial[i]) {
      ...
      //judge
      if (judge_type[i,t] == 0) {
        summ = evV[i,stimR[i,t]] + evV[i,stimL[i,t]] -mean(evV[i,])*2;
